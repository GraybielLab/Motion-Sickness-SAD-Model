...
%%%%%GENERATION
      if plot_catenated
                figure(Fig_AsymCatenator_seed.fignum+ns)
                ixhd1=Subject(ns).Head==1;
                ixhd2=Subject(ns).Head==2;
                    if stoppedgen==0   
                        xhead=x+asymoffset+extramismatchasymoffsetG;
                        plot(xhead(ixhd1),rate(ixhd1),'-','marker',markertypeG,'color',Head(1).color);hold on;
                        plot(xhead(ixhd2),rate(ixhd2),'-','marker',markertypeG,'color',Head(2).color);
                    else
                        plot(x+asymoffset+extramismatchasymoffsetG,rate,'-','color',mc('blue'),'marker',markertypeG);
                    end
                    hold on
...
%                 SubjectCycle(ns).Head(ihd).Gen_R2(cycle)=Subject(ns).HD(ihd).Rsqr(cycle);
                ...
                set(gcf,'name',['S' num2str(ns) '_' sID '_All_NR_AsymCat_vs_Time'])
                xlm=get(gca,'xlim');
      end
...
elseif gendecay==2
       if plot_catenated
                ...
                %     subplot(sperplot, nc,(subplotix-1)*nc+cycle)
                    if stoppeddecay==0   
                        plot(x+asymoffset+extramismatchasymoffsetD,rate,'g-','marker',markertypeD,'markerfacecolor',headupdnmarkerfacecolor);
                    else
                        plot(x+asymoffset+extramismatchasymoffsetD,rate,'r-','marker',markertypeD,'markerfacecolor',headupdnmarkerfacecolor);
                        %plotfitD=0;
                    end
                    hold on
