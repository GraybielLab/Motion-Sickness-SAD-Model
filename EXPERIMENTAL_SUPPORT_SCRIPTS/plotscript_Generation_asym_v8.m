...
if plot_subjects_in_groups_GENvsT
                        ...
                        figure(Fig_GEN_NRvsTIMEasym.fignum+ixf)
                        icycle=cycle;if icycle>10 icycle=icycle-10;figure(10*Fig_GEN_NRvsTIMEasym.fignum+ixf);end
                        Stxt=['S' num2str((ixf-1)*sperplot+1) '-S' num2str(ixf*sperplot)];
                        
                        subplot(sperplot, nc , (subplotix-1)*nc + icycle)
                        xlim([x(1) x(end)])
                        if stoppedgen==0   
                              %plot(x,rate,'g-','marker',markertypeG);COMMENTED on 04/30/21
                              plot(x(ixhd1),rate(ixhd1),'-','marker',markertypeG,'color',Head(1).color);hold on;
                              plot(x(ixhd2),rate(ixhd2),'-','marker',markertypeG,'color',Head(2).color);

                        else
                             plot(x,rate,'r-','marker',markertypeG);
                        end
                        title(['S' num2str(ns) '-' sID '   ' type ' ' stoptitG]);hold on
                        xlabel(Fig_GEN_NRvsTIMEasym.xlabel)
                        ylabel(Fig_GEN_NRvsTIMEasym.ylabel)
                        ylim([0 10])
                        xlm=get(gca,'xlim');
                        set(gcf,'color','w');
                        set(gcf,'position',[0 5 1921 981])
                        set(gcf,'name',[Fig_GEN_NRvsTIMEasym.name ' (' Stxt ')'])  
...
if plot_subjects_in_groups_dGAINvsT
           ixf=floor(ns/(sperplot+.1))+1;
           figure(Fig_dGAIN_NRvsTIMEasym.fignum+ixf)
           icycle=cycle;if icycle>10 icycle=icycle-10;figure(10*Fig_GEN_NRvsTIMEasym.fignum+ixf);end
           Stxt=['S' num2str((ixf-1)*sperplot+1) '-S' num2str(ixf*sperplot)];
            subplot(sperplot,nc,(subplotix-1)*nc+icycle)
%             xlim([x(1) x(end)])
            if stoppedgen==0        
                 %plot(x(2:end),iS,'g-','marker',markertypeG);COMMENTED on 04/30/21
                 x_hd=x(2:end);
                 plot(x_hd(ixhd1(2:end)),iS(ixhd1(2:end)),'-','marker',markertypeG,'color',Head(1).color);hold on;
                 plot(x_hd(ixhd2(2:end)),iS(ixhd2(2:end)),'-','marker',markertypeG,'color',Head(2).color);
            else
            ...
            hv(1,0);
            title(['S' num2str(ns) '-' sID '   ' type ' ' stoptitG]);hold on
            xlabel(Fig_dGAIN_NRvsTIMEasym.xlabel)
            ylabel(Fig_dGAIN_NRvsTIMEasym.ylabel)
            ylim([-2 4])
            xlm=get(gca,'xlim');
            set(gcf,'color','w');
            set(gcf,'position',[46 5 1869 973])
            set(gcf,'name',[Fig_dGAIN_NRvsTIMEasym.name ' (' Stxt ')'])  
end
